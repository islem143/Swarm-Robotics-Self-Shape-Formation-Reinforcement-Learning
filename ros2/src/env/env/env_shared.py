...
class Env(Node):
    ...
    def crashs(self):
        ...
        if (self.min_ldss_dist[0] < 0.13 or self.min_ldss_dist[1] < 0.13):

            return True
        return False

    def goal_reached(self):
        dist = self.get_distance(self.positions[0], self.goal_cords[0])
        dist2 = self.get_distance(self.positions[1], self.goal_cords[0])
        if (dist >= 1 and dist < 1.30 and dist2>=1 and dist2<=1.30):
    ...
    def get_reward(self):
        rewards = [0, 0]
        for index in range(self.num_agents):
            distance = self.get_distance(self.positions[0], self.goal_cords[0])
            rewards[index] -=(distance-1)+0.3

            rewards[index] += -2*np.abs(self.goal_angles[index])

            # if (self.goal_angles[index] < 0.40):
            #     rewards[index] += 5
            if (self.min_ldss_dist[index] < 0.45 and self.min_ldss_angle[index] in [21, 22, 23, 0, 1, 2]):
                rewards[index] -= 10
            else:
                rewards[index] += 0.1
