...
class ACNetwork():
    ...
    def create_critic_model(self):
      ...
      #  action_out =  keras.layers.BatchNormalization()(action_out)
        ...
        out = keras.layers.Dense(256, kernel_regularizer=keras.regularizers.l2(0.01),activation="relu",kernel_initializer=keras.initializers.GlorotNormal())(concat)
        out=keras.layers.Dropout(0.2)(out)
       # out = keras.layers.BatchNormalization()(out)
        out = keras.layers.Dense(256, activation="relu",kernel_regularizer=keras.regularizers.l2(0.01),kernel_initializer=keras.initializers.GlorotNormal())(out)
        out=keras.layers.Dropout(0.2)(out)
        # out = keras.layers.BatchNormalization()(out)
