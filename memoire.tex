...
\subsection{Ros and Neural network Implemantation}
...
We have followed the openai gym implemation of diffrent rl problms.
In the training phase, we have defined three nodes. The first node  interact with gazebo and allows us to reset the simulation for example.

The second node is the environemnt node which will collect data from the simulated sensors of gazebo and get the positions of the robots. He is also 
responsible of detecting collision and goal reaching in order to reset the simulation if needed and setting the rewards. 

Finally, the third node is reponsible for getting state and rewards from the environement then passing them to the models to be trained. 

For the models, they were implemented in a seperate python class.
