...
The  goal of each robot is to  maximize his return by reaching the goal target and avoiding collisons with the wall and other  robots. We will use Deep Q learning to evaluate the states, and after that the robots will take the actions that  maximize their return.
...
We'll utilize the polar coordinates (R,$\Phi$), where R and $\Phi$ stand for the target's radius and angle, respectively. After assigning each robot a goal position, we will convert it to homogeneous coordinates so  it may be reached  by it. The desired shapes will be formed by defining the distance and the angle for each robot from the center of the formation.
